...
class ModelTrainer:
    ...
    def initate_model_trainer(self,train_arr,test_arr):
        try:
            ...
            best_acc = max(model_report.values())
            best_model_name = list(model_report.keys())[list(model_report.values()).index(best_acc)]

            best_model = models[best_model_name]

            if best_acc < 0.6: # this is the threshold value
            ...
            model_acc_score = accuracy_score(y_test,predicated)
            return(
                best_model,
                model_acc_score
            )
